...
980X / 681 / 680 會合併九巴及城巴各自提供的 ETA；任何一個來源失敗都不會令另一個來源失效。城巴使用已核對的固定站碼 `001950`（馬鞍山市中心，22.424134522091, 114.23169341053）。980X 以城巴 `outbound` 方向及灣仔／Wan Chai 目的地欄位核對；681 / 680 則以 `inbound` 方向及目的地欄位核對中環／金鐘方向。不同營辦商時間相近的班次會保留為兩班獨立巴士。
